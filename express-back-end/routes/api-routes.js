...
router.get("/users/:id/preferences", (req, res) => {
  const userId = req.params.id;
  const query = `
    SELECT genders.value AS gender,  drinks.value AS drink, exercises.value AS exercise, dating_goals.value AS dating_goal, location, min_age, max_age,  min_height_in_cm, max_height_in_cm
      FROM preferences
